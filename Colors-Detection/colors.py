...
# Function to calculate mininum distance from all colors and get the most matching color 
def getColorName(R, G, B):
	minimum = 10000
	for i in range(len(csv)):
		d = abs(R - int(csv.loc[i, 'R'])) + abs(B - int(csv.loc[i, 'B']))
		if(d <= minimum ):
			minimum = d
...
while(1):
	...
	if (clicked):
		...
		# cv2.rectangle(image, startpoint, endpoint, color, thickeness) - 1 fills entire rectangle
		...
		# For the light colours we will display text in black colour
		if(r+g+b >= 600):
